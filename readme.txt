...
7.3 Сигнатуры функций и примеры
...
def is_cbs (lisp_reference: str) -> bool: *возвращаемое значение можно поменять на int, чтобы возвращать -1 в случае
получения некорректной строки, а 1 и 0 в качестве ответа на вопрос*
7.2 – Сигнатура второй функции
def need_to_move (lisp_reference: str) -> int:
Пример 1: «()()», «(())()», «(())» и «()» являются правильными скобочными последовательностями,
а «)(», «()(» и «)))» – нет.
